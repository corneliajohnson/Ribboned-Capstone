...
namespace Ribboned.Repositories
{
    public class SnagRepository : ISnagRepository
    {
        ...
        public List<Snag> GetAllUserSnags(int userId)
        {
            return _context.Snag
                .Include(s => s.Ribbon)
                .Where(s => s.Ribbon.Category.UserProfileId == userId).ToList();
        }
        ...
        {
            return _context.Snag.Where(s => s.RibbonId == ribbonId).OrderBy(s => s.Seconds).ToList();
        }
